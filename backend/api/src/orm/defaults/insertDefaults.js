...
const demo_images_host =
  "https://localhost:8080/public/files/bee2728b-b74c-4501-90c1-e387294a86fb/images/";
...
export async function insertDefaults() {
  const userAdmin = ArUser.create({
    ...
    admin: true,
    password: await passwordHash("admin"),
  });

  for (let i = 0; i < 2; i++) {
    const user = await ArUser.create({
      username: "user" + i,
      email: "user" + i + "@gmail.com",
      admin: false,
      password: await passwordHash("user" + i),
    });
  }
  ...
  const adminProject2 = await ArProject.create({
    ...
    title: "Autre projet",
    description:
      "D'autres projets sont possibles ! Le site n'est pas limité à une seule maquette. Ce projet peut par exemple être dédié à l'évolution d'un bâtiment en particulier.",
    unit: "Couleur",
  ...
  const adminProject = await ArProject.create({
    ...
    description:
      "Le musée des Soieries Bonnet se trouve dans l'ancienne usine de l'entreprise Bonnet à Jujurieux. De 1835 à 2001, ce site a produit différents textiles (en pariculier de la soie). En devenant musée, on a pu préserver les collections, les bâtiments, et aussi le savoir-faire de cette industrie. Le musée possède un fonds exceptionnel de plus de 300 000 objets présents dans les bâtiments d'origine, et que tu peux admirer au travers de cette maquette numérique.",
    pictureUrl:
      "public/files/bee2728b-b74c-4501-90c1-e387294a86fb/images/cover1720533869014.jpg",
    unit: "Année",
  ...
  const adminProjectScene1 = await ArScene.create({
    ...
    description:
      "L'histoire commence en 1834, où Claude Bonnet fait l'acquisition d'une maison bourgeoise, accompagnée des ses dépendances et d'un grand terrain",
    index: 0,
  });
  ...
  const adminProjectScene2 = await ArScene.create({
    ...
    projectId: "bee2728b-b74c-4501-90c1-e387294a86fb",
    description:
      "Il y fait construire de nouveaux bâtiments jusqu'en 1845. À ce moment, le site est déjà capable de produire de grande quantités de soie, mais la croissance ne fait que commencer...",
    index: 1,
  });
  ...
  const adminProjectScene3 = await ArScene.create({
    ...
    projectId: "bee2728b-b74c-4501-90c1-e387294a86fb",
    description:
      "Les premiers ouvriers à avoir travaillé aux usines Bonnet de Jujurieux sont des ouvrières appelées « pensionnaires ». Découvre leur quotidien en appuyant sur le bouton play en bas à droite de l'écran.",
    index: 2,
  });
  ...
  const labelScene3_1 = await ArLabel.create({
    ...
    position: {
      x: -0.21489120689924243,
      y: 0.14627271364429353,
      z: -0.5102086835890118,
    },
    text:
      '<img src="' +
      demo_images_host +
      'img1720533869019.png" alt="img" style="width: 100%"> <p>Les pensonnaires sont des jeunes filles (entre 13 et 20 ans) qui travaillent à la préparation des fils de soie (filature + moulinage). Comme leur nom l\'indique, elles vivent sur le site. Leur quotidien est partagé entre le travail à l\'usine et la vie au pensionnat.</p> <iframe src="/assets/project/audio.mp3" allow="autoplay" style=\'display: none\'></iframe>',
    timestampStart: 1000,
    timestampEnd: 8000,
    sceneId: "487f3cd4-b56c-4150-92eb-f71ac81ffa75",
  });
  ...
  const labelScene3_2 = await ArLabel.create({
    ...
    position: {
      x: -0.21395685784430915,
      y: 0.14428607270691834,
      z: -0.5099197471725173,
    },
  ...
  const labelScene3_3 = await ArLabel.create({
    ...
    position: {
      x: -0.2094429658937141,
      y: 0.03858790690100975,
      z: -0.4720492673551239,
    },
    text:
      '<h2>6h00</h2> <img src="' +
      demo_images_host +
      'img1720533869018.png" alt="img" style="width: 100%"> <p>Sortie du pensionnat. Les ouvrières se dirrigent vers leur lieu de travail</p>',
  ...
  const labelScene3_4 = await ArLabel.create({
    ...
    position: {
      x: -0.49286627720905796,
      y: 0.19039767577046013,
      z: -0.2675729058486698,
    },
    text:
      '<h2>6h00 - 19h00</h2>     <img src="' +
      demo_images_host +
      'img1720533869021.png" alt="img" style="width: 100%">     <p>Certaines ouvrières travaillent à l\'atelier de moulinage</p>',
  ...
  const labelScene3_5 = await ArLabel.create({
    ...
    position: {
      x: -0.3283896279700032,
      y: 0.16386768413392627,
      z: 0.1543917637701666,
    },
    text:
      '<h2>6h00 - 19h</h2> <img src="' +
      demo_images_host +
      'img1720533869022.png" alt="img" style="width: 100%"> <p>D\'autres vont à l\'atelier de tissage</p>',
  ...
  const labelScene3_6 = await ArLabel.create({
    ...
    position: {
      x: 0.16015580215545883,
      y: 0.12825367754646322,
      z: 0.036562552542710916,
    },
    text:
      '<img src="' +
      demo_images_host +
      'img1720533869017.png" alt="img" style="width: 100%"> <p>Les ouvrières travaillent 11h par jour. En cas de problème, elles peuvent se rendre à l\'infirmerie</p>',
  ...
  const labelScene3_7 = await ArLabel.create({
    ...
    position: {
      x: -0.21513652846980857,
      y: 0.14527367759473336,
      z: -0.505120763624475,
    },
  ...
  const labelScene3_8 = await ArLabel.create({
    ...
    position: {
      x: 0.010159509006652929,
      y: 0.050822096361347546,
      z: 0.07922896356551723,
    },
    text:
      '<img src="' +
      demo_images_host +
      'img1720533869020.png" alt="img" style="width: 100%"> <p>Le soir, les ouvrières retournent au pensionnat ou vont profiter de leur temps libre</p>',
  ...
  const labelScene3_9 = await ArLabel.create({
    ...
    position: {
      x: 0.2032504270699614,
      y: 0.03471952448255418,
      z: -0.11373036762109905,
    },
    text:
      '<img src="' +
      demo_images_host +
      'img1720533869015.png" alt="img" style="width: 100%">',
  ...
  const labelScene3_10 = await ArLabel.create({
    ...
    position: {
      x: -0.06845833190936906,
      y: 0.10253275541151594,
      z: 0.1966693160928878,
    },
    text:
      '<img src="' +
      demo_images_host +
      'img1720533869016.png" alt="img" style="width: 100%">',
  ...
  const labelScene3_11 = await ArLabel.create({
    ...
    position: {
      x: -0.2146897792797725,
      y: 0.14570254300079752,
      z: -0.5078721108245865,
    },
